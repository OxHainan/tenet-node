	...
	Executor: NativeExecutionDispatch + 'static,
{
	...
	impl sp_inherents::InherentDataProvider for MockTimestampInherentDataProvider {
		async fn provide_inherent_data(
			...
			TIMESTAMP.with(|x| {
				*x.borrow_mut() += tenet_runtime::SLOT_DURATION;
...
pub async fn build_full(
	...
	new_full::<tenet_runtime::RuntimeApi, TemplateRuntimeExecutor>(config, eth_config, sealing)
		.await
}
...
> {
	...
	let PartialComponents {
		...
		other,
		..
	} = new_partial::<tenet_runtime::RuntimeApi, TemplateRuntimeExecutor, _>(
		config,
		eth_config,
