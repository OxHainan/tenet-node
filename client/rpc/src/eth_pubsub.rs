...
use tp_ethereum::{BlockV2 as EthereumBlock, TransactionV2 as EthereumTransaction};
// Substrate
use sc_client_api::{
...
// Frontier
use tc_mapping_sync::{EthereumBlockNotification, EthereumBlockNotificationSinks};
use tc_rpc_core::{
	types::{
...
impl EthSubscriptionResult {
	...
	pub fn logs(
		block: EthereumBlock,
		receipts: Vec<tp_ethereum::Receipt>,
		...
		for (receipt_index, receipt) in receipts.into_iter().enumerate() {
			let receipt_logs = match receipt {
				...
				| tp_ethereum::Receipt::EIP1559(d) => d.logs,
			};
					...
					let stream = block_notification_stream
						.filter_map(move |notification| {
							if notification.is_new_best {
								...
								let schema = tc_storage::onchain_storage_schema(
									client.as_ref(),
									substrate_hash,
								);
