...
// Frontier
use tc_rpc_core::{types::*, EthFilterApiServer};
	...
	A: ChainApi<Block = B> + 'static,
{
		...
		match path {
			...
			FuturePath::Block { last, next } => {
				...
				for n in last..next {
					...
					let schema =
						tc_storage::onchain_storage_schema(client.as_ref(), substrate_hash);
	...
	async fn logs(&self, filter: Filter) -> RpcResult<Vec<Log>> {
		...
		if let Some(hash) = filter.block_hash {
			...
			let schema = tc_storage::onchain_storage_schema(client.as_ref(), substrate_hash);
	...
	BE: Backend<B> + 'static,
{
	...
	while current_number <= to {
		...
		let schema = tc_storage::onchain_storage_schema(client, substrate_hash);
