			...
			(Some(block), Some(statuses), Some(receipts)) => {
				...
				let (logs, logs_bloom, status_code, cumulative_gas_used, gas_used) =
					if !block_info.is_eip1559 {
						...
						match receipt {
							tp_ethereum::Receipt::Legacy(ref d) => {
								let index = core::cmp::min(receipts.len(), index + 1);
								let cumulative_gas: u32 = receipts[..index]
									.iter()
									.map(|r| match r {
										tp_ethereum::Receipt::Legacy(d) => Ok(d.used_gas.as_u32()),
						...
						match receipt {
							tp_ethereum::Receipt::Legacy(ref d)
							| tp_ethereum::Receipt::EIP2930(ref d)
							| tp_ethereum::Receipt::EIP1559(ref d) => {
								...
								let gas_used = if index > 0 {
									...
									let previous_gas_used = match previous_receipt {
										...
										| tp_ethereum::Receipt::EIP1559(d) => d.used_gas,
									};
				...
				Ok(Some(Receipt {
					...
					transaction_type: match receipt {
						tp_ethereum::Receipt::Legacy(_) => U256::from(0),
