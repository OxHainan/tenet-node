...
pub fn sync_block<Block: BlockT, C, BE>(
	...
	backend: &tc_db::kv::Backend<Block>,
	header: &Block::Header,
) -> Result<(), String>
where
	...
	BE: Backend<Block>,
{
	...
	match fp_consensus::find_log(header.digest()) {
		Ok(log) => {
			let gen_from_hashes = |hashes: Hashes| -> tc_db::kv::MappingCommitment<Block> {
				tc_db::kv::MappingCommitment {
					block_hash: substrate_block_hash,
					ethereum_block_hash: hashes.block_hash,
					ethereum_transaction_hashes: hashes.transaction_hashes,
				}
			};
			let gen_from_block = |block| -> tc_db::kv::MappingCommitment<Block> {
			...
			match log {
				...
				Log::Post(post_log) => match post_log {
					...
					PostLog::BlockHash(expect_eth_block_hash) => {
						let schema =
							tc_storage::onchain_storage_schema(client, substrate_block_hash);
	...
	C::Api: EthereumRuntimeRPCApi<Block>,
{
		...
		.map_err(|e| format!("{:?}", e))?
	{
		...
		let block_hash = block
			...
			.header
			.hash();
		let mapping_commitment = tc_db::kv::MappingCommitment::<Block> {
			block_hash: substrate_block_hash,
...
pub fn sync_one_block<Block: BlockT, C, BE>(
	...
	overrides: Arc<OverrideHandle<Block>>,
	frontier_backend: &tc_db::kv::Backend<Block>,
