...
impl FromStr for MetaKey {
	...
	fn from_str(input: &str) -> Result<MetaKey, Self::Err> {
		let tips = str::from_utf8(tc_db::kv::static_keys::CURRENT_SYNCING_TIPS).unwrap();
...
pub struct MetaDb<'a, B: BlockT> {
	cmd: &'a FrontierDbCmd,
	backend: Arc<tc_db::kv::Backend<B>>,
}

impl<'a, B: BlockT> MetaDb<'a, B> {
	pub fn new(cmd: &'a FrontierDbCmd, backend: Arc<tc_db::kv::Backend<B>>) -> Self {
		Self { cmd, backend }
	}
