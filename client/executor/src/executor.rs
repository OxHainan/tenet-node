...
impl<D: NativeExecutionDispatch> NativeElseWasmExecutor<D> {
	...
	pub fn allow_missing_host_functions(&mut self, allow_missing_host_functions: bool) {
		if let Some(ref mut wasm) = self.wasm {
			wasm.allow_missing_host_functions = allow_missing_host_functions
		}
	}
}
...
impl<D: NativeExecutionDispatch + 'static> CodeExecutor for NativeElseWasmExecutor<D> {
	...
	fn call(
		...
		if let Some(ref wasm) = &self.wasm {
			let on_chain_heap_alloc_strategy = if wasm.ignore_onchain_heap_pages {
				...
				runtime_code
					...
					.unwrap_or_else(|| wasm.default_onchain_heap_alloc_strategy)
			};

			let heap_alloc_strategy = match context {
				CallContext::Offchain => wasm.default_offchain_heap_alloc_strategy,
				CallContext::Onchain => on_chain_heap_alloc_strategy,
			};
...
mod tests {
	...
	fn native_executor_registers_custom_interface() {
		...
		my_interface::HostFunctions::host_functions()
			...
			.for_each(|function| {
				assert_eq!(
					extract_host_functions(&executor.wasm.to_owned())
						.iter()
						.filter(|f| f == &function)
						.count(),
					2
