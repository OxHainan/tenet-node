...
#[tokio::test]
async fn deploy_tenet() -> Result<(), Box<dyn std::error::Error>> {
    let bytecode = 
        fs::read_to_string(TENET_BYTECODE_FILE).expect("Failed to read bytecode file");
    ...
    loop {
        match web3.eth().transaction_receipt(transaction_hash.clone()).await? {
            Some(receipt) => {
                println!("Contract TENET deployed at address: {}",
...
#[tokio::test]
async fn test_register_tee() -> Result<(), Box<dyn std::error::Error>> {
    ...
    register_tee(
        peer_id,quote_size,quote_buf,sup_size,sup_buf,tee_public_key,p2p_connect_info
    ).await?;
    Ok(())
}
async fn register_tee(
    ...
    let abi = 
        fs::read_to_string(TENET_BYTECODE_ABI).expect("Failed to read ABI file");
    let contract = 
        Contract::from_json(web3.eth(), contract_address, abi.as_bytes())?;
    let result = contract.call("registerTEE", (
        peer_id,
        quote_size,
        quote_buf,
        sup_size,
...
#[tokio::test]
async fn test_register_api() -> Result<(), Box<dyn std::error::Error>> {
    let peer_id = String::from("peeridtest");
...
pub async fn update_challenge_bytes(
    ...
    options.gas = Some(4_000_000.into());
    println!("pom:{:?}, bytes:{:?}", pom.to_json(), pom.to_json().into_bytes());

    let result = contract.call(
        "updateChallengeBytes", 
        (peer_id, pom.challenge_id.as_bytes().to_vec(), pom.to_json().into_bytes(), sig), 
        account, 
        options,
    ).await?;
