...
mod tests {
	...
	fn limited_child_kill_off_by_one_works() {
		...
		let mut ext = Ext::new(&mut overlay, &backend, None);
		let r = ext
			.kill_child_storage(&child_info, Some(0), None)
			.deconstruct();
		...
		let r = ext
			.kill_child_storage(&child_info, Some(1), None)
			.deconstruct();
	...
	fn test_compact(remote_proof: StorageProof, remote_root: &sp_core::H256) -> StorageProof {
		let compact_remote_proof = remote_proof
			.into_compact_proof::<BlakeTwo256>(*remote_root)
			.unwrap();
	...
	fn prove_read_and_proof_check_works_inner(state_version: StateVersion) {
		...
		assert_eq!(
			local_result3.into_iter().collect::<Vec<_>>(),
			vec![(b"dummy".to_vec(), None)]
		);
	}
	...
	fn child_read_compact_stress_test() {
		...
		for i in 0..50u32 {
			...
			let backend = TrieBackendBuilder::wrap(&trie)
				.with_recorder(Default::default())
				.build();
