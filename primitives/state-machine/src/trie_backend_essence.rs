...
impl<S: TrieBackendStorage<H>, H: Hasher, C: TrieCacheProvider<H>> TrieBackendEssence<S, H, C> {
	...
	fn with_recorder_and_cache<R>(
		...
		let storage_root = storage_root.unwrap_or(self.root);
		let mut cache = self
			.trie_node_cache
			.as_ref()
		...
		#[cfg(feature = "std")]
		{
			...
			let recorder = recorder
				.as_mut()
				.map(|recorder| recorder as &mut dyn TrieRecorder<H::Out>);
	...
	H::Out: Codec + Ord,
{
	...
	pub fn storage(&self, key: &[u8]) -> Result<Option<StorageValue>> {
		...
		let decrypt = |key, v: &[u8]| {
			#[cfg(feature = "std")]
			{
				tp_io::crypto::decrypt(v, key).unwrap()
			}
	...
	pub fn storage_root<'a>(
		...
		for (key, val) in delta {
			if let Some(val) = val {
				#[cfg(feature = "std")]
				deltas.push((key, Some(tp_io::crypto::encrypt(val, key).unwrap())));
